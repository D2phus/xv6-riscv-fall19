...
binit(void)
{
  ...
  // 暂时将所有buffer放到0号bucket中，之后在bget中进行分配
...
bget(uint dev, uint blockno)
{
  ...
  // 如果在h对应的bucket中没有找到，那么需要到其他bucket中找
  // 此时原来bucket的锁还没有释放，因为我们在其他bucket中找到buffer后，还要将其插入到原bucket中
  int nh=(h+1)%NBUCKETS; // nh表示下一个要探索的bucket，当它重新变成h，说明所有的buffer都busy
  while(nh!=h){
